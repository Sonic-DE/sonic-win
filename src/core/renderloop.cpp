...
void RenderLoopPrivate::notifyFrameCompleted(std::chrono::nanoseconds timestamp, std::optional<RenderTimeSpan> renderTime, PresentationMode mode, OutputFrame *frame)
{
    if (s_printDebugInfo && !m_debugOutput) {
        // X11 has one shared render loop, without an associated output.
        const QString outputName = output ? output->name() : QStringLiteral("x11");
        m_debugOutput = std::fstream(qPrintable("kwin perf statistics " + outputName + ".csv"), std::ios::out);
